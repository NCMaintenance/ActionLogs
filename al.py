...
def load_and_merge_data(uploaded_file) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if 'Risk Rating' in merged_df.columns:
            ...
            merged_df['Risk Rating'] = merged_df['Risk Rating'].replace(rating_map)
        ...
        if 'Risk Impact Category' in merged_df.columns:
            ...
            for pattern, replacement in category_replacements.items():
                # FIX: Replaced inplace=True with direct assignment
                merged_df['Risk Impact Category'] = merged_df['Risk Impact Category'].replace(to_replace=pattern, value=replacement, regex=True)
        ...
        # FIX: Convert Unique Risk ID to string to prevent Arrow serialization errors
        if 'Unique Risk ID' in merged_df.columns:
            merged_df['Unique Risk ID'] = merged_df['Unique Risk ID'].astype(str)
            logger.info("Converted 'Unique Risk ID' column to string type.")
...
def assign_gemini_topics_batch(_df: pd.DataFrame, api_key: str) -> pd.DataFrame:
    ...
    try:
        ...
        # FIX: Updated model name based on error log
...
def create_data_quality_report(df: pd.DataFrame) -> None:
    ...
    with col1:
        ...
        styled_df = completeness_df.style.map(
            colour_completeness, 
            subset=['Completeness %']
...
def run_professional_dashboard():
    ...
    # FIX: Download NLTK resources at the start
