...
def parse_due_date(date_str: str) -> Optional[datetime]:
    """
    Intelligently parses a string like "Q4 2025" or "q1 2024" into a datetime object.
    
    Args:
        date_str (str): The string to parse.
    
    Returns:
        Optional[datetime]: The parsed date, or None if parsing fails.
    """
    ...
    # Regex to find "Q" or "q" followed by a number, and a four-digit year
...
def load_and_merge_data(uploaded_file) -> Optional[pd.DataFrame]:
    ...
    try:
        with show_loading_spinner("Loading and validating Excel file..."):
            ...
            # --- NEW FEATURE: Risk Timeline Data Processing ---
            if 'Due Date' in merged_df.columns:
                merged_df['Due Date'] = merged_df['Due Date'].astype(str).apply(parse_due_date)
            else:
                merged_df['Due Date'] = pd.NaT
...
def create_risk_timeline_view(df: pd.DataFrame) -> None:
    """
    New view for the Risk Timeline.
    Shows the number of risks due over time, grouped by quarter and filtered by priority.
    """
    st.markdown('<div class="section-header"><h3>⏳ Risk Timeline Analysis</h3></div>', unsafe_allow_html=True)
    
    # Filter for risks with a valid due date
    df_timeline = df.dropna(subset=['Due Date']).copy()
    
    if df_timeline.empty:
        show_info_message("No risks with a valid 'Due Date' found in the dataset for this analysis.")
        return

    # Create a 'quarter_year' column for grouping
    df_timeline['quarter_year'] = df_timeline['Due Date'].dt.to_period('Q').astype(str)
    df_timeline = df_timeline.sort_values('Due Date')
    
    # Get a list of unique risk ratings for the filter
    risk_ratings = sorted(df_timeline['Risk Rating'].unique())
    selected_ratings = st.multiselect(
        "Filter by Risk Priority:",
        options=risk_ratings,
        default=risk_ratings
    )
    
    if not selected_ratings:
        show_info_message("Please select at least one risk priority level.")
        return
        
    df_filtered = df_timeline[df_timeline['Risk Rating'].isin(selected_ratings)]
    
    if df_filtered.empty:
        show_info_message("No data matches the selected risk priorities for the timeline.")
        return

    # Count risks by quarter and rating
    timeline_counts = df_filtered.groupby(['quarter_year', 'Risk Rating']).size().reset_index(name='count')
    ...
    fig = px.bar(
        ...
        labels={'quarter_year': 'Quarter', 'count': 'Number of Risks'},
    ...
    fig.update_layout(
        ...
        xaxis={'categoryorder': 'array', 'categoryarray': sorted(df_timeline['quarter_year'].unique())},
        legend_title_text='Risk Priority'
    )
    
    st.plotly_chart(fig, use_container_width=True)

def create_comparative_analysis_view(df: pd.DataFrame) -> None:
    """
    New view for Comparative Analysis.
    Allows side-by-side comparison of two or more facilities.
    """
    ...
    selected_facilities = st.multiselect(
        "Select two or more facilities for comparison:",
        options=all_facilities,
        default=all_facilities[:2]
    )
    
    if len(selected_facilities) < 2:
        show_info_message("Please select at least two facilities to enable comparative analysis.")
        return

    num_facilities = len(selected_facilities)
    
    # Create a dynamic number of columns for side-by-side view
    cols = st.columns(num_facilities)
    
    for i, facility in enumerate(selected_facilities):
        df_facility = df[df['HSE Facility'] == facility].copy()
        
        with cols[i]:
            st.markdown(f"**🏥 {facility}**")
            st.markdown("---")
            
            # Key metrics for the facility
            total_risks = len(df_facility)
            high_risks = len(df_facility[df_facility['Risk Rating'] == 'High'])
            st.metric(label="Total Risks", value=total_risks)
            st.metric(label="High Priority Risks", value=high_risks, delta_color="inverse")
            
            # Risk Priority Distribution
            st.subheader("Risk Priority")
            rating_counts = df_facility['Risk Rating'].value_counts()
            fig_rating = px.bar(
            ...
            fig_rating.update_layout(showlegend=False, xaxis_title="", yaxis_title="Count")
            st.plotly_chart(fig_rating, use_container_width=True)
            
            # AI-Generated Topic Distribution
            st.subheader("AI Topics")
            if 'AI-Generated Topic' in df_facility.columns:
                ai_topic_counts = df_facility['AI-Generated Topic'].value_counts()
                fig_ai_donut = px.pie(
                ...
                fig_ai_donut.update_traces(textposition='inside', textinfo='percent')
...
def create_advanced_analytics_view(df: pd.DataFrame) -> None:
    """
    New view for Advanced Analytics.
    Includes Risk Correlation Matrix and Bubble Chart.
    """
    st.markdown('<div class="section-header"><h3>🔬 Advanced Analytics</h3></div>', unsafe_allow_html=True)
    
    # --- Bubble Chart ---
    st.subheader("⚪ Risk Category Bubble Chart")
    
    # Check for required columns
    required_cols = ['AI-Generated Topic', 'HSE Facility', 'Risk Rating']
    if not all(col in df.columns for col in required_cols):
        show_info_message("The dataset is missing required columns for the Bubble Chart.")
    else:
        # Aggregate data for the bubble chart
        bubble_data = df.groupby('AI-Generated Topic').agg(
            facilities_affected=('HSE Facility', 'nunique'),
            total_risks=('HSE Facility', 'count'),
        ...
        fig_bubble = px.scatter(
            ...
            log_y=True,
            size_max=80,
            title='Widespread and Severe Risk Categories',
            labels={
                'facilities_affected': 'Number of Facilities Affected',
                'total_risks': 'Total Number of Risks',
                'high_priority_risks': 'High Priority Risks'
            }
        )
        fig_bubble.update_layout(
            xaxis_title="Number of Facilities Affected (log scale)",
            yaxis_title="Total Number of Risks (log scale)"
        )
        st.plotly_chart(fig_bubble, use_container_width=True)
    
    st.markdown("---")
    
    # --- Risk Correlation Matrix (Heatmap) ---
    st.subheader("🔥 Risk Correlation Matrix")
    
    # Check for required columns
    if not all(col in df.columns for col in ['HSE Facility', 'Risk Rating', 'AI-Generated Topic']):
        show_info_message("The dataset is missing required columns for the Correlation Matrix.")
    else:
        # Create a pivot table for the heatmap
        heatmap_data = pd.pivot_table(
            df,
            index='AI-Generated Topic',
            columns='Risk Rating',
        ...
        # Ensure all risk ratings are present in the columns
        for rating in ['High', 'Medium', 'Low']:
            if rating not in heatmap_data.columns:
                heatmap_data[rating] = 0
        ...
        fig_heatmap = px.imshow(
            heatmap_data,
            text_auto=True,
            title='Risk Category vs. Priority Correlation',
            color_continuous_scale='Blues',
            labels=dict(x="Risk Priority Level", y="AI-Generated Topic", color="Count")
        )
        fig_heatmap.update_xaxes(side="top")
        st.plotly_chart(fig_heatmap, use_container_width=True)


def create_sankey_diagram(df: pd.DataFrame) -> None:
...
def run_professional_dashboard(df: pd.DataFrame, view_selection: str):
    """
    Main dashboard function to render the selected view.
    """
    ...
    elif view_selection == "Risk Timeline":
        create_risk_timeline_view(df_filtered)
        
    elif view_selection == "Comparative Analysis":
        create_comparative_analysis_view(df) # Pass original df to allow full facility selection
        
    elif view_selection == "Advanced Analytics":
        create_advanced_analytics_view(df_filtered)
        
    # Always show the filtered dataset and download button
...
def main():
    """Enhanced main function with professional authentication and view selection"""
    ...
    if st.session_state.authenticated:
        ...
        # Main view selection
        st.sidebar.markdown("---")
        st.sidebar.markdown("### 👁️ Select View")
        ...
        try:
            run_professional_dashboard(df, view_selection)
        except Exception as e:
